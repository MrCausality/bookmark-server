...
class Shortener(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if "longuri" not in params or "shortname" not in params:
            ...
            self.send_header('Content-type', 'text/plain; charset=utf-8')
            self.end_headers()
            self.wfile.write('You must fill out all fields!'.encode())
            return False
        ...
        if CheckURI(longuri):
            ...
            self.send_response(404,message="That site doesn't exist!")
            self.send_header('Content-type', 'text/plain; charset=utf-8')
            self.end_headers()
            self.wfile.write(
                "That URI '{}' doesn't exist. Sorry!".format(longuri).encode())

if __name__ == '__main__':
    port = int(os.environ.get('PORT', 8000)) #Use PORT environmental variable if present
    server_address = ('', port)
